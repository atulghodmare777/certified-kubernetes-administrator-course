...
- In the k8s if want to modify the runtime commands which will be different from the default then we have to change, command field override the entrypoint instruction from docker file and args filed override the cmd instruction from docker file, this is to modify the default behaviour.
